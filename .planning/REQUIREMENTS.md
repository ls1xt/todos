...
- [x] **TDD-02**: bats test cases written for infrastructure: pre-commit hook validation (valid files, missing frontmatter, bad filename) and template structure
- [x] **TDD-03**: bats test cases written for core scripts: `add` (all flags, non-interactive), `done`/`block`/`drop` (move + commit), error cases (zero matches, multiple matches)
- [x] **TDD-04**: bats test cases written for discovery scripts: `list` (all filter combinations), `review` (default and --days), `session` (tmux attach/create)
